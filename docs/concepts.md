...
The `Lumineer` class has shortcuts to both `can()` and `hasRole()` for the currently logged in user:

```php
Lumineer::hasRole('role-name');
Lumineer::can('permission-name');
...
```
The `Lumineer` class has a shortcut to `ability()` for the currently logged in user:

```php
Lumineer::ability('admin,owner', 'create-post,edit-user');
